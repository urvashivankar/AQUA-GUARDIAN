...
def get_government_jurisdiction_reports(current_user = Depends(get_current_user)) -> List[Report]:
    ...
    try:
        ...
        if not jurisdiction_res.data:
            ...
            if email in ["green@ngo.org", "vdr.manager@gmail.com", "ahm.manager@gmail.com", "ahmedabad.city@gov.in"]:
                target_city = "Ahmedabad" if ("ahm" in email or "ahmedabad" in email) else "Vadodara"
                logger.info(f"🚨 DEMO MODE (Reports): Overriding jurisdiction for {email} -> {target_city}")
                
                demo_jur = supabase.table("government_jurisdictions").select("id, government_code, city_name").ilike("city_name", f"%{target_city}%").execute()
                if demo_jur.data:
